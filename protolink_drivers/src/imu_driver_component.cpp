...
ImuDriverComponent::ImuDriverComponent(const rclcpp::NodeOptions &options)
    : Node("imu_driver_component", options),
      params_(imu_driver::ParamListener(get_node_parameters_interface())
                  .get_params()),
      publisher_(create_publisher<hardware_communication_msgs__SimpleImu::
                                      hardware_communication_msgs__SimpleImu>(
          "geopose", 1)),
      subscriber_(io_, params_.port, [this](const auto &msg) {
        publisher_->publish(convert(msg));
      }) {}
} // namespace protolink_drivers
